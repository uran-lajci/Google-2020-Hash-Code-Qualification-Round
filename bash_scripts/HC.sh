...
OUTPUT_BASE_DIR="$PROJECT_ROOT/output_instances/HC"
RESULTS_FILE="$PROJECT_ROOT/results/HC.csv"
...
find "$INPUT_BASE_DIR" -type f -name "*.txt" | while read INPUT_FILE; do
    ...
    OUTPUT=$(java -cp "$CLASS_DIR" Main "$INPUT_FILE" "$OUTPUT_PATH" 4 10000 2>&1)
